...
`google.adk.integrations.agent_registry` imports both the A2A SDK and the Agent Identity auth provider at module scope, so importing `AgentRegistry` raises `ImportError` on a core-only install. Install both the `a2a` and `agent-identity` extras:
...
- `header_provider` (Callable, optional): A callable that takes a ReadonlyContext and returns a dictionary of custom headers to be included in requests made by the [McpToolset](/tools-custom/mcp-tools/#mcptoolset-class) that `get_mcp_toolset` returns, to the target MCP server. These headers do not affect calls to the Agent Registry API itself, and they do not affect requests made by [RemoteA2aAgent](/a2a/quickstart-consuming-go/#quickstart-consuming-a-remote-agent-via-a2a). For those requests, pass an authenticated `httpx.AsyncClient` to `get_remote_a2a_agent`, as shown in [Remote A2A Agents](#remote-a2a-agents).
